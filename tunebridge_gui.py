...
def _setup_native_menus(window):
    ...
    class _TBMenuHandler(NSObject):
        ...
        def importMusic_(self, sender): _js(window, 'App.openImportWizard()')
    ...
    file_m.addItem_(sep())
    file_m.addItem_(mi('Import Music…', 'importMusic:', 'i', target=handler))
    file_m.addItem_(mi('Import Playlist…', 'importPl:', target=handler))
    file_m.addItem_(sep())

...
function _orgWizRenderDone(s) {
  ...
  const moved = s.moved || 0, skipped = s.skipped || 0, errCount = errors.length;
  ...
  const WARNSVG = `<svg width="15" height="15" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M8 2L1.5 14h13L8 2z"/><path d="M8 6.5v3"/><circle cx="8" cy="11.5" r="0.6" fill="currentColor"/></svg>`;
  const errListHtml = errCount ? `<div class="orgw-error-list"><div class="orgw-error-hdr">${WARNSVG}<span>${errCount} file${errCount !== 1 ? 's' : ''} could not be ${verb}</span></div><div class="orgw-error-rows">${errors.map(e => { const fname = (e.path || '').split('/').pop() || e.path || 'Unknown file'; const msg = (e.error || 'Unknown error').replace(/^\[Errno \d+\]\s*/, ''); return `<div class="orgw-error-row"><span class="orgw-err-file">${_esc(fname)}</span><span class="orgw-err-msg">${_esc(msg)}</span></div>`; }).join('')}</div></div>` : '';
  if (runWrap) runWrap.innerHTML = `<div class="orgw-applied${errCount && !moved ? ' orgw-applied--error' : ''}"><span class="orgw-ap-glyph">${errCount && !moved ? WARNSVG : CHECKSVG}</span><div><div class="orgw-ap-t1">${moved} file${moved !== 1 ? 's' : ''} ${verb}</div><div class="orgw-ap-t2">Organised with "${_esc(tName)}"${errCount ? ` · ${errCount} error${errCount !== 1 ? 's' : ''}` : ''}.</div></div></div><div class="orgw-done-stats"><span class="orgw-stat orgw-stat-move"><span class="orgw-stat-n">${moved}</span><span class="orgw-stat-l">${moved === 1 ? 'file moved' : 'files moved'}</span></span>${skipped ? `<span class="orgw-stat-div"></span><span class="orgw-stat"><span class="orgw-stat-n" style="color:var(--text-muted)">${skipped}</span><span class="orgw-stat-l">skipped</span></span>` : ''}${errCount ? `<span class="orgw-stat-div"></span><span class="orgw-stat orgw-stat-conflict"><span class="orgw-stat-n">${errCount}</span><span class="orgw-stat-l">error${errCount !== 1 ? 's' : ''}</span></span>` : ''}</div><div class="orgw-done-actions"><button class="orgw-btn orgw-btn-primary orgw-btn-lg" id="orgw-done-library">View in library</button><button class="orgw-btn orgw-btn-lg" id="orgw-done-undo">Undo this run</button><button class="orgw-btn orgw-btn-ghost orgw-btn-lg" id="orgw-done-again">Organise more</button></div>${errListHtml}`;

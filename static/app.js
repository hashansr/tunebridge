  ...
  rail.innerHTML = items.map(item => {
    ...
    return `<div class="home-card" onclick="App.homeOpenItem(${onClickArgs})" role="button" tabindex="0"
      ...
      <div class="${artClass}"${artStyle}>
        ...
        ${items.length > 1 ? `<div class="home-pin-drag-handle" onclick="event.stopPropagation()" oncontextmenu="event.stopPropagation()" title="Drag to reorder"><span class="queue-drag-icon"></span></div>` : ''}
      </div>
...
function _showCtxMenu(x, y, tracks, label, favTarget = null, finderTarget = null, pinTarget = null) {
  ...
  if (findLyricsItem && findLyricsSep && findLyricsLabel) {
    const missing = tracks.filter(t => !t.has_lyrics);
...
async function ctxFindLyrics() {
  ...
  try {
    const res = await fetch('/api/lyrics/fetch', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ track_ids: missing.map(t => t.id) }),
    }).then(r => r.json());
    if (res.error) {
      liveToast.finish(res.error, 'error');
      return;
    }
    if (res.queued) {
    ...
    if (found) {
      ...
      liveToast.finish('No lyrics found');
    }
    await refreshCurrentLibraryView();
    if (typeof Player !== 'undefined' && Player.currentTrack) {
      _updateLyricsButtonForTrack(Player.currentTrack);
    }
  } catch (e) {
    liveToast.finish('Error fetching lyrics', 'error');
  }
}
...
const _GEAR_ICON_EJECT = `<svg width="14" height="14" viewBox="-32 0 512 512" fill="currentColor" aria-hidden="true"><path d="M448 384v64c0 17.673-14.327 32-32 32H32c-17.673 0-32-14.327-32-32v-64c0-17.673 14.327-32 32-32h384c17.673 0 32 14.327 32 32zM48.053 320h351.886c41.651 0 63.581-49.674 35.383-80.435L259.383 47.558c-19.014-20.743-51.751-20.744-70.767 0L12.67 239.565C-15.475 270.268 6.324 320 48.053 320z"/></svg>`;
const _GEAR_ICON_EDIT = `<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M17.0671 2.27157C17.5 2.09228 17.9639 2 18.4324 2C18.9009 2 19.3648 2.09228 19.7977 2.27157C20.2305 2.45086 20.6238 2.71365 20.9551 3.04493C21.2864 3.37621 21.5492 3.7695 21.7285 4.20235C21.9077 4.63519 22 5.09911 22 5.56761C22 6.03611 21.9077 6.50003 21.7285 6.93288C21.5492 7.36572 21.2864 7.75901 20.9551 8.09029L20.4369 8.60845L15.3916 3.56308L15.9097 3.04493C16.241 2.71365 16.6343 2.45086 17.0671 2.27157Z"/><path d="M13.9774 4.9773L3.6546 15.3001C3.53154 15.4231 3.44273 15.5762 3.39694 15.7441L2.03526 20.7369C1.94084 21.0831 2.03917 21.4534 2.29292 21.7071C2.54667 21.9609 2.91693 22.0592 3.26314 21.9648L8.25597 20.6031C8.42387 20.5573 8.57691 20.4685 8.69996 20.3454L19.0227 10.0227L13.9774 4.9773Z"/></svg>`;
...
function _setSettingsActionStatus(action, opts = {}) {
  const tone = opts.tone || 'info';
  const show = !!(opts.show || opts.title);
  document.querySelectorAll(`[data-settings-action-status="${action}"]`).forEach(node => {
    node.style.display = show ? 'inline-flex' : 'none';
    node.textContent = '';
    node.title = opts.title || '';
    node.classList.remove('has-text');
...
function _renderDupGroupCard(g) {
  ...
  const rows = (g.tracks || []).map((t, i) => {
    ...
    // Finder button — path stored in data-* to avoid onclick string-escaping issues
    const finderBtn = `<button class="dup-finder-btn"
        data-finder-path="${esc(path)}"
        data-finder-scope="${isDap ? 'dap' : 'library'}"
        title="Open in Finder"
        onclick="App._dupOpenFinder(this)">
  ...
  return `<div class="dup-group-card" id="dup-group-${esc(g.key)}">
    ...
    <table class="dup-group-table tb-table tb-table-density-compact">
      ...
      <thead><tr>
        <th>Action</th>
        <th>Format</th><th>Bitrate</th><th>Dur</th><th>Size</th>
        <th>Path</th>
        <th></th>
      </tr></thead>
      <tbody>${rows}</tbody>
...
/**
 * Reveal a track's file in macOS Finder.
 * Called with the button element so the path is read from data-finder-path
 * (avoiding any onclick-string escaping issues with special chars in paths).
 */
...
/* ── Public API ─────────────────────────────────────────────────────── */
/* ── Lyrics ─────────────────────────────────────────────────────────── */
let _lyricsViewOpen = false;
let _lyricsTrackId = null;
let _lyricsLines = [];
let _lyricsGaps = [];
let _lyricsSynced = false;
let _lyricsSyncThrottle = 0;
...
function openLyricsView() {
  ...
  _lyricsViewOpen = true;
  _lyricsUserScrolled = false;
  document.getElementById('player-lyrics-btn')?.classList.add('lyrics-active');
  const scroll = document.getElementById('lyrics-scroll');
  if (scroll && !scroll._lyricsScrollHandler) {
    scroll._lyricsScrollHandler = () => {
      _lyricsUserScrolled = true;
      clearTimeout(_lyricsScrollResumeTimer);
      _lyricsScrollResumeTimer = setTimeout(() => { _lyricsUserScrolled = false; }, 4000);
    };
    scroll.addEventListener('scroll', scroll._lyricsScrollHandler, { passive: true });
  }
  const cur = typeof Player !== 'undefined' ? Player.currentTrack : null;
  if (cur) _loadLyricsForTrack(cur.id, cur);
...
function _lyricsEscHandler(e) {
  if (e.key === 'Escape') closeLyricsView();
}

async function _loadLyricsForTrack(trackId, track) {
  _lyricsTrackId = trackId;
  _lyricsLines = [];
  _lyricsGaps = [];
  _lyricsSynced = false;
  const scroll = document.getElementById('lyrics-scroll');
  const bg = document.getElementById('lyrics-bg');
  if (bg && track?.artwork_key) bg.style.backgroundImage = `url(/api/artwork/${encodeURIComponent(track.artwork_key)})`;
  else if (bg) bg.style.backgroundImage = '';
  if (scroll) scroll.innerHTML = '<div class="lyrics-empty-state">Loading...</div>';
  try {
    ...
    const data = await res.json();
    const parsed = _parseLyrics(data.content || '', !!data.is_synced);
    _lyricsSynced = parsed.is_synced;
    _lyricsLines = parsed.lines;
    _renderLyrics(parsed);
  } catch (_) {
    if (scroll) scroll.innerHTML = '<div class="lyrics-empty-state">Error loading lyrics</div>';
  }
}
...
function _renderLyrics(parsed) {
  ...
  if (!parsed.lines.length) {
    scroll.innerHTML = '<div class="lyrics-empty-state">No timed lyrics available</div>';
    return;
  }
  _lyricsGaps = _buildLyricsGaps(parsed.lines);
  const parts = ['<div class="lyrics-spacer"></div>'];
  ...
  parts.push('<div class="lyrics-spacer"></div>');
  scroll.innerHTML = parts.join('');
}

function _buildLyricsGaps(lines) {
  const gaps = [];
  const introThreshold = 3000;
  const gapThreshold = 4500;
  const gapLeadIn = 1500;
  if (lines[0]?.ms > introThreshold) gaps.push({ startMs: 0, endMs: lines[0].ms, beforeLineIdx: 0 });
  for (let i = 0; i < lines.length - 1; i++) {
    const gap = lines[i + 1].ms - lines[i].ms;
    if (gap > gapThreshold) gaps.push({ startMs: lines[i].ms + gapLeadIn, endMs: lines[i + 1].ms, afterLineIdx: i });
  }
  return gaps;
}

function _lyricsGapHtml(gap) {
  return `<div class="lyrics-gap" data-start="${Math.round(gap.startMs)}" data-end="${Math.round(gap.endMs)}"><span></span><span></span><span></span></div>`;
}

function _syncLyricsToTime(seconds) {
  if (!_lyricsViewOpen || !_lyricsSynced || !_lyricsLines.length) return;
  const now = performance.now();
  if (now - _lyricsSyncThrottle < 100) return;
  _lyricsSyncThrottle = now;
  const ms = seconds * 1000;
  let activeIdx = -1;
  for (let i = 0; i < _lyricsLines.length; i++) {
    if (_lyricsLines[i].ms <= ms) activeIdx = i;
    else break;
  }
  const scroll = document.getElementById('lyrics-scroll');
  if (!scroll) return;
...
function _updateLyricsButtonForTrack(track) {
  const btn = document.getElementById('player-lyrics-btn');
  if (!btn) return;
  const hasLyrics = !!(track && (track.has_lyrics || track.lyrics_status === 'synced' || track.lyrics_status === 'plain'));
...
async function loadLyricsSettings() {
  try {
    const [settings, stats, status] = await Promise.all([
      fetch('/api/lyrics/settings').then(r => r.json()).catch(() => ({ service: 'lrclib' })),
      fetch('/api/lyrics/stats').then(r => r.json()).catch(() => ({})),
      fetch('/api/lyrics/status').then(r => r.json()).catch(() => ({ status: 'idle' })),
    ]);
...
function _updateLyricsStatsRow(stats) {
  const el = document.getElementById('lyrics-stats-text');
  if (!el) return;
...
async function onLyricsServiceChange(service) {
  ...
  toast('Lyrics service saved');
}

function toggleLyricsAdvanced() {
  const body = document.getElementById('lyrics-advanced-body');
  const chevron = document.getElementById('lyrics-advanced-chevron');
  if (!body) return;
...
async function cancelLyricsBulk() {
  await fetch('/api/lyrics/cancel', { method: 'POST' }).catch(() => {});
  _stopLyricsBulkPolling();
  loadLyricsSettings();
}

function _startLyricsBulkPolling() {
  if (_lyricsBulkPoller) return;
  _lyricsBulkPoller = setInterval(_pollLyricsBulkStatus, 1000);
  _pollLyricsBulkStatus();
}

function _stopLyricsBulkPolling() {
  if (_lyricsBulkPoller) { clearInterval(_lyricsBulkPoller); _lyricsBulkPoller = null; }
}

async function _pollLyricsBulkStatus() {
  try {
    const s = await fetch('/api/lyrics/status').then(r => r.json());
    _updateLyricsBulkBanner(s);
    if (['done', 'cancelled', 'error', 'idle'].includes(s.status)) {
      _stopLyricsBulkPolling();
      fetch('/api/lyrics/stats').then(r => r.json()).then(_updateLyricsStatsRow).catch(() => {});
      if (s.status === 'done') refreshCurrentLibraryView();
    }
  } catch (_) {}
}

function _updateLyricsBulkBanner(s) {
...
function _orgPopulateTemplateSelect() {
  const sel = document.getElementById('org-template-select');
  if (!sel) return;
  sel.innerHTML = '<option value="">-- choose saved template --</option>';
  for (const t of (_orgModal.templates || [])) {
    const opt = document.createElement('option');
...
async function orgNext() {
  if (_orgModal.step === 1) {
    await _orgRunPreview();
  } else if (_orgModal.step === 2) {
    await _orgStartApply();
  }
}

function orgBack() {
  if (_orgModal.step === 2) _orgGoToStep(1);
}
...
async function _orgRunPreview() {
  ...
  try {
    let data;
    if (_orgModal.mode === 'import') {
      if (!_orgModal.sources.length) { toast('Add at least one source file or folder'); btnNext.disabled = false; btnNext.textContent = 'Preview'; return; }
      data = await api('/organizer/import/scan', {
        method: 'POST',
        body: { template: tpl, sources: _orgModal.sources.map(s => ({ type: s.type, path: s.path })) },
      });
    } else {
      data = await api('/organizer/preview', { method: 'POST', body: { template: tpl, mode: 'reorganize' } });
    }
    _orgModal.previewData = data;
    _orgRenderPreview(data);
    _orgGoToStep(2);
  } catch (e) {
...
function _orgRenderPreview(data) {
  const summary = data.summary || {};
  const el = document.getElementById('org-preview-summary');
  el.innerHTML = `
    <span class="org-summary-pill">${summary.moves || 0} moves</span>
    <span class="org-summary-pill org-summary-pill--warn">${summary.conflicts || 0} conflicts</span>
    <span class="org-summary-pill org-summary-pill--info">${summary.missing_metadata || 0} warnings</span>
    <span class="org-summary-pill org-summary-pill--muted">${summary.same_path || 0} unchanged</span>
  `;
  document.getElementById('org-chip-all').textContent = summary.total || 0;
...
function orgSetPreviewFilter(filter) {
  _orgModal.previewFilter = filter;
  document.querySelectorAll('#org-preview-chips .sw-chip').forEach(b => {
    b.classList.toggle('sw-chip--active', b.dataset.filter === filter);
  });
  _orgPaintPreviewList();
}

function _orgPaintPreviewList() {
  const list = document.getElementById('org-preview-list');
  if (!list || !_orgModal.previewData) return;
  const entries = _orgModal.previewData.entries || [];
  const filter = _orgModal.previewFilter;
  const visible = entries.filter(e => {
    if (filter === 'moves') return !e.same_path;
    if (filter === 'conflicts') return e.conflict;
    if (filter === 'warnings') return e.warnings && e.warnings.length;
    return true;
  });

  if (!visible.length) {
    list.innerHTML = '<div class="org-empty">No entries match this filter.</div>';
    return;
  }

  const CHUNK = 80;
  const rows = visible.slice(0, CHUNK);
  list.innerHTML = rows.map(e => {
    let cls = 'org-preview-row';
    if (e.conflict) cls += ' org-preview-row--conflict';
    else if (e.warnings && e.warnings.length) cls += ' org-preview-row--warning';
    ...
    return `<div class="${cls}">
      <span class="org-row-old" title="${_esc(src)}">${_esc(_basename(src))}</span>
      <svg class="org-row-arrow" width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><path d="M10 6L8.59 7.41 13.17 12l-4.58 4.59L10 18l6-6z"/></svg>
      <span class="org-row-new" title="${_esc(dst)}">${_esc(dst)}</span>
      ${badges.join('')}
    </div>`;
  }).join('');

  if (visible.length > CHUNK) {
    list.insertAdjacentHTML('beforeend', `<div class="org-empty" style="text-align:center;padding:8px 0">Showing first ${CHUNK} of ${visible.length} entries</div>`);
  }
}
...
async function _orgStartApply() {
  if (!_orgModal.previewData) return;
  const entries = _orgModal.previewData.entries || [];
  const plan = entries.filter(e => !e.same_path || _orgModal.mode === 'import');
  ...
  try {
    const res = await api('/organizer/apply', {
      method: 'POST',
      body: {
        template: _orgModal.template,
        conflict_policy: _orgModal.conflictPolicy,
        mode: _orgModal.mode,
        plan: plan,
      },
    });
    _orgModal.runId = res.run_id;
    _orgStartPolling();
  } catch (e) {
    _orgGoToStep(2);
...
function _orgStartPolling() {
  _orgModal.pollTimer = setInterval(_orgPoll, 800);
}

async function _orgPoll() {
  const s = await api('/organizer/status').catch(() => null);
  if (!s) return;
  _orgRenderProgress(s);
  if (s.status === 'done' || s.status === 'done_with_errors' || s.status === 'error') {
    clearInterval(_orgModal.pollTimer);
    _orgModal.pollTimer = null;
    _orgRenderDone(s);
    _orgGoToStep(4);
    _orgLoadUndoBar();
...
function _orgRenderProgress(s) {
  const n = s.progress || 0;
  const total = s.total || 1;
  const pct = Math.round((n / total) * 100);
  document.getElementById('org-progress-n').textContent = n;
  document.getElementById('org-progress-pct').textContent = pct + '%';
  document.getElementById('org-progress-bar').style.width = pct + '%';
  const cur = document.getElementById('org-current-file');
  if (cur) cur.textContent = s.current || '';
  if (s.errors && s.errors.length) {
    const errEl = document.getElementById('org-error-list');
    errEl.innerHTML = s.errors.slice(-5).map(e =>
      `<div class="org-error-row"><span>${_esc(e.path || '')}</span><span class="org-error-msg">${_esc(e.error || '')}</span></div>`
    ).join('');
  }
}

function _orgRenderDone(s) {
  const el = document.getElementById('org-done-summary');
  const moved = s.moved || 0;
  const skipped = s.skipped || 0;
  const errs = (s.errors || []).length;
  el.innerHTML = `
    <div class="org-done-icon">
      <svg width="32" height="32" viewBox="0 0 24 24" fill="currentColor" style="color:var(--accent)"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>
    </div>
    <div class="org-done-title">${_orgModal.mode === 'import' ? 'Import' : 'Organise'} complete</div>
    <div class="org-done-stats">
      <span class="org-summary-pill">${moved} ${_orgModal.mode === 'import' ? 'imported' : 'moved'}</span>
      ${skipped ? `<span class="org-summary-pill org-summary-pill--muted">${skipped} skipped</span>` : ''}
      ${errs ? `<span class="org-summary-pill org-summary-pill--conflict">${errs} errors</span>` : ''}
    </div>
...
async function orgAddFiles() {
  try {
    ...
    for (const p of (res.paths || [])) {
      if (!_orgModal.sources.find(s => s.path === p)) {
        const name = p.split('/').pop();
        _orgModal.sources.push({ type: 'file', path: p, label: name });
      }
    }
    _orgRenderSourceList();
  } catch (e) {
    toast('File picker not available — type the path manually');
  }
}
...
async function orgAddFolder() {
  try {
    const res = await api('/browse/folder', { method: 'POST' });
    if (!res.path) return;
    if (!_orgModal.sources.find(s => s.path === res.path)) {
      const name = res.path.split('/').pop();
      _orgModal.sources.push({ type: 'folder', path: res.path, label: name + '/' });
    }
    _orgRenderSourceList();
  } catch (e) {
    toast('Folder picker not available');
  }
}

function orgRemoveSource(idx) {
  _orgModal.sources.splice(idx, 1);
  _orgRenderSourceList();
}

function _orgRenderSourceList() {
  const el = document.getElementById('org-source-list');
  if (!el) return;
  if (!_orgModal.sources.length) { el.innerHTML = ''; return; }
  el.innerHTML = _orgModal.sources.map((s, i) =>
    `<div class="org-source-item">
      <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor" style="flex-shrink:0;opacity:.5">${s.type === 'folder' ? '<path d="M10 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2h-8l-2-2z"/>' : '<path d="M12 3v10.55c-.59-.34-1.27-.55-2-.55-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4V7h4V3h-6z"/>'}</svg>
      <span title="${_esc(s.path)}">${_esc(s.label)}</span>
      <button class="org-source-remove" onclick="App.orgRemoveSource(${i})" title="Remove">&#x2715;</button>
    </div>`
  ).join('') + `<button class="org-source-clear" onclick="App.orgClearSources()">Clear All</button>`;
}

function orgClearSources() {
  _orgModal.sources = [];
  _orgRenderSourceList();
}
...
async function _orgLoadUndoBar() {
  const data = await api('/organizer/undo/last').catch(() => null);
  const bar = document.getElementById('org-undo-bar');
  if (!bar) return;
  if (data && data.run_id && data.count > 0) {
    document.getElementById('org-undo-summary').textContent =
      `Last run: ${data.count} file${data.count !== 1 ? 's' : ''} moved`;
    bar.style.display = 'flex';
  } else {
    bar.style.display = 'none';
  }
...
async function orgRollback() {
  if (!await _showConfirm({
    ...
    message: 'This will move all files back to their original paths. Playlists and the library index will be restored.',
    okText: 'Rollback',
    danger: false,
  })) return;
  ...
  try {
    const res = await api('/organizer/undo/rollback', { method: 'POST' });
    toast(`Rollback complete: ${res.restored} restored, ${res.errors} errors`);
  } catch (e) {
...
function _orgTmplRenderList() {
  ...
  if (!el) return;
  if (!_orgModal.templates.length) {
    el.innerHTML = '<div class="org-empty" style="padding:16px 0;text-align:center">No saved templates. Create one below.</div>';
    return;
  }
  el.innerHTML = _orgModal.templates.map(t => `
    <div class="org-tmpl-row">
      <div class="org-tmpl-info">
        <span class="org-tmpl-name">${_esc(t.name)}${t.is_default ? ' <span class="org-default-badge">default</span>' : ''}</span>
        <code class="org-tmpl-code">${_esc(t.template)}</code>
      </div>
      <div class="org-tmpl-actions">
        ${!t.is_default ? `<button class="btn-secondary btn-sm" onclick="App.orgTmplSetDefault('${t.id}')">Set Default</button>` : ''}
        <button class="btn-secondary btn-sm" onclick="App.orgTmplDelete('${t.id}')">Delete</button>
      </div>
    </div>
  `).join('');
}
...
async function orgTmplSaveNew() {
  ...
  try {
    await api('/organizer/templates', { method: 'POST', body: { name, template } });
    _orgModal.templates = await api('/organizer/templates').catch(() => []);
    _orgTmplRenderList();
    document.getElementById('org-tmpl-new-form').style.display = 'none';
...
async function orgTmplDelete(id) {
  if (!await _showConfirm({ title: 'Delete template?', message: 'This cannot be undone.', okText: 'Delete', danger: true })) return;
...
async function orgTmplSetDefault(id) {
  ...
  _orgModal.templates = await api('/organizer/templates').catch(() => []);
  _orgTmplRenderList();
  // Save as default_organizer_template in settings
  const t = _orgModal.templates.find(x => x.id === id);
  if (t) await api('/settings', { method: 'PUT', body: { default_organizer_template: t.template } }).catch(() => {});
  toast('Default template updated');
}

async function _orgLoadActiveTemplateDisplay() {
  const settings = await api('/settings').catch(() => ({}));
  const tpl = settings.default_organizer_template || ORGANIZER_DEFAULT_TEMPLATE;
  const code = document.getElementById('org-active-template-code');
  const wrap = document.getElementById('org-active-template-display');
  if (code) code.textContent = tpl;
  if (wrap) wrap.style.display = tpl ? '' : 'none';
}
...
function _updateAnalysisBanner(s) {
  ...
  const _doneSvg   = `<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path fill-rule="evenodd" clip-rule="evenodd" d="M1 12C1 5.92487 5.92487 1 12 1C18.0751 1 23 5.92487 23 12C23 18.0751 18.0751 23 12 23C5.92487 23 1 18.0751 1 12ZM18.4158 9.70405C18.8055 9.31268 18.8041 8.67952 18.4127 8.28984L17.7041 7.58426C17.3127 7.19458 16.6796 7.19594 16.2899 7.58731L10.5183 13.3838L7.19723 10.1089C6.80398 9.72117 6.17083 9.7256 5.78305 10.1189L5.08092 10.8309C4.69314 11.2241 4.69758 11.8573 5.09083 12.2451L9.82912 16.9174C10.221 17.3039 10.8515 17.301 11.2399 16.911L18.4158 9.70405Z"/></svg>`;

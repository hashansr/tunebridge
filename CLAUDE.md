...
- [x] iPod library browse + keep/remove selection — inside `#sw-step-ipod`, after a scan: "Playlists" and "Artists" `.sw-group-card` sections styled identically to the DAP wizard's Step 3 review UI (same `.sw-group-card`/`.sw-tree-artist`/`.sw-tree-album`/`.sw-review-row`/custom `.sw-check` classes), collapsed by default, top-level lists paginated at 25 items/page via `.sw-page-bar` (`SW_IPOD_BROWSE_PAGE_SIZE`) — small enough that pagination shows up even for a typical playlist count, not just the 300+-artist case. Each level has a checkbox defaulting to checked ("keep"); Album/Artist checkboxes are tri-state and cascade to their descendants (full re-render on every change, same as Step 3's `_swRenderReview`); unchecking is independent per playlist vs per track. "Continue" advances to `#sw-step-ipod-sync`, a dedicated step that runs the existing `/api/ipods/<id>/sync/scan` check, merges in the client-computed `remove_track_ids`/`remove_playlist_ids`, and executes everything in one `/api/ipods/<id>/sync/execute` call (same route the Gear iPod detail page's add/remove actions already use)
...
Sync button (⟳ arrows icon) in sidebar bottom bar opens `#view-sync` — a full-page 5-step wizard (`_sw`/`sw*` functions in `static/app.js`), not the `#sync-modal` markup (that's dead/orphaned CSS from an earlier modal-based implementation). Step 1: device picker, categorised into "DAPs" and "iPods" sections, mount status shown as green "Connected" / grey "Offline" chips. Step 2: scanning with animated progress bar and live log. Step 3: review changes — filter chips + two sections (copy to device ↑ / copy to local ↓) with scrollable checkbox lists, folder path in muted colour, filename in brighter, "Select all" per section. Step 4: syncing with file-by-file progress. Step 5: done, showing summary + any errors. Selecting an iPod in Step 1 branches into two dedicated steps instead of steps 2-5, both in the same wizard chrome: `#sw-step-ipod` (browse/select) and `#sw-step-ipod-sync` (execute). `#sw-step-ipod`: Scan button, then (once scanned) a "Playlists" / "Artists" review UI reusing Step 3's exact CSS classes and DOM patterns (`_swBuildIpodPlaylistsCard`, `_swBuildIpodArtistsCard`, `_swBuildIpodArtistHierarchy`, `_swBuildIpodSongRow` in `static/app.js` — `.sw-group-card`, `.sw-tree-artist`/`.sw-tree-album`/`.sw-tree-songs`, `.sw-review-row`, custom `.sw-check`/`.sw-tree-check` checkboxes, `.sw-page-bar` pagination). All levels collapsed by default; checkboxes default to checked ("keep"); unchecking a song/album/artist marks its track(s) for removal (tri-state cascade computed fresh on every render via `_swIpodGroupCheckState`, same full-re-render approach as Step 3's `_swRenderReview` rather than incremental DOM patching); unchecking a playlist marks the playlist itself for removal, independently of its tracks. Interactive elements use `data-*` attributes read by `*El` bridge functions (`swIpodToggleTreeExpandEl`, `swIpodToggleTreeNodeEl`, `swIpodToggleSongEl`, `swIpodTogglePlaylistEl`) rather than embedding values into onclick strings — same pattern Step 3 itself uses, needed because artist/album/playlist names can contain quotes. "Continue" moves to `#sw-step-ipod-sync`, which auto-runs `/api/ipods/<id>/sync/scan` (the existing add-missing-tracks check), merges in the client-side `remove_track_ids`/`remove_playlist_ids` computed from the checkboxes (`_swComputeIpodRemovals`), and on "Sync now" posts everything in one `/api/ipods/<id>/sync/execute` call — the same route the Gear iPod detail page's individual add/remove actions already use, which already supported combined add+remove before this wizard step existed.
...
Deferred, not yet built: live space required/available/left calculation during selection. Browse, explicit keep/remove selection (playlists, artists, albums, songs), and combined removal+transcode+addition in one sync step are now built (see "Sync Feature" below).

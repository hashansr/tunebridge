...
- [x] iPod library browse + keep/remove/add selection — inside `#sw-step-ipod`, after a scan: four `.sw-group-card` sections styled identically to the DAP wizard's Step 3 review UI (same `.sw-group-card`/`.sw-tree-artist`/`.sw-tree-album`/`.sw-review-row`/custom `.sw-check` classes). "Playlists" and "Artists" show what's already on the device — uncheck to mark for removal. "Songs to add" and "Playlists to add" show what's in the local library but not yet on the device (excluding anything already there — matching reuses `ipod/sync_planner.py::compute_sync_plan`'s existing fuzzy title/artist/album key, not reinvented) — checked by default, uncheck to skip. The artist-tree renderer is shared between the "on device" and "to add" cards via a `scope` parameter (`'device'`/`'add'`, see `IPOD_GROUP_CONFIG` in `static/app.js`) rather than duplicated. All levels collapsed by default, top-level lists paginated at 25 items/page via `.sw-page-bar` (`SW_IPOD_BROWSE_PAGE_SIZE`). Album/Artist checkboxes are tri-state and cascade to their descendants (full re-render on every change, same as Step 3's `_swRenderReview`). "Continue" advances to `#sw-step-ipod-sync`, which needs no further scanning — both the add and remove selections are already known — and executes everything in one `/api/ipods/<id>/sync/execute` call (same route the Gear iPod detail page's add/remove actions already use, which already supported combined add+remove+playlist-sync before this wizard step existed)
...
Sync button (⟳ arrows icon) in sidebar bottom bar opens `#view-sync` — a full-page 5-step wizard (`_sw`/`sw*` functions in `static/app.js`), not the `#sync-modal` markup (that's dead/orphaned CSS from an earlier modal-based implementation). Step 1: device picker, categorised into "DAPs" and "iPods" sections, mount status shown as green "Connected" / grey "Offline" chips. Step 2: scanning with animated progress bar and live log. Step 3: review changes — filter chips + two sections (copy to device ↑ / copy to local ↓) with scrollable checkbox lists, folder path in muted colour, filename in brighter, "Select all" per section. Step 4: syncing with file-by-file progress. Step 5: done, showing summary + any errors. Selecting an iPod in Step 1 branches into two dedicated steps instead of steps 2-5, both in the same wizard chrome: `#sw-step-ipod` (browse/select) and `#sw-step-ipod-sync` (execute). `#sw-step-ipod`: Scan button, then (once scanned) four review cards reusing Step 3's exact CSS classes and DOM patterns (`_swBuildIpodPlaylistsCard`, `_swBuildIpodArtistsCard`, `_swBuildIpodArtistHierarchy`, `_swBuildIpodSongRow`, `_swBuildIpodAddPlaylistsCard` in `static/app.js` — `.sw-group-card`, `.sw-tree-artist`/`.sw-tree-album`/`.sw-tree-songs`, `.sw-review-row`, custom `.sw-check`/`.sw-tree-check` checkboxes, `.sw-page-bar` pagination):
  - "Playlists" / "Artists" — what's on the device now (`_swLoadIpodLibraryBrowse`, fetches `/api/ipods/<id>/tracks` + `/playlists`). Uncheck a song/album/artist to mark its track(s) for removal (tri-state cascade via `_swIpodGroupCheckState`); uncheck a playlist to remove the playlist itself, independently of its tracks.
  - "Songs to add" / "Playlists to add" — what's in the local library but not yet on the device, fetched via `_swLoadIpodAddPlan`/`_swPollIpodAddPlan`, which POST `/api/ipods/<id>/sync/scan` (triggered automatically right after the device scan, in parallel with the "on device" fetch) and read the enriched `plan.tracks_to_add`/`plan.playlists_to_create`/`plan.playlists_to_update` — matching against what's already on the device is `ipod/sync_planner.py::compute_sync_plan`'s existing fuzzy title/artist/album key, not reinvented here. Checked by default; unchecking skips that item. The artist-tree card/row builders (`_swBuildIpodArtistsCard`, `_swBuildIpodArtistHierarchy`, `_swBuildIpodSongRow`) take a `scope` argument (`'device'` | `'add'`, config in `IPOD_GROUP_CONFIG`) and are shared between the "on device" and "to add" cards rather than duplicated — device track ids are numbers (`device_track_id`), local library track ids are MD5 hex strings (`t.id`), so the shared code always resolves the right id via `_swIpodTrackIdOf(scope, track)` rather than assuming one type.
  All levels collapsed by default. Interactive elements use `data-*` attributes read by `*El` bridge functions (`swIpodToggleTreeExpandEl`, `swIpodToggleTreeNodeEl`, `swIpodToggleSongEl`, `swIpodTogglePlaylistEl`, `swIpodToggleAddPlaylistEl`) rather than embedding values into onclick strings — same pattern Step 3 itself uses, needed because artist/album/playlist names can contain quotes. "Continue" moves to `#sw-step-ipod-sync`, which needs no further network round-trip — both the add selections (`_swComputeIpodAdditions`) and remove selections (`_swComputeIpodRemovals`) were already computed client-side in the previous step — and on "Sync now" posts everything in one `/api/ipods/<id>/sync/execute` call with `track_ids`/`playlist_ids` (add) and `remove_track_ids`/`remove_playlist_ids` (remove) — the same route the Gear iPod detail page's individual actions already use. Selecting a playlist to add does not require also checking its member songs individually: the backend already expands `playlist_ids` to include any missing member tracks automatically (see `ipod_sync_execute` in `app.py`), so the two selections are independent by design, same as the device-side keep/remove split.
...
Deferred, not yet built: live space required/available/left calculation during selection. Browse, explicit keep/remove/add selection (playlists, artists, albums, songs — both what's already on the device and what's available to add from the local library), and combined removal+transcode+addition+playlist-sync in one sync step are now built (see "Sync Feature" below).
...
- **Follow-up fix — scores were still overly positive**: after the overhaul above shipped, real owned IEMs all scored 80-94% for every genre with zero differentiation. Root cause: `k=0.08` in `_score_iem_17d` (inherited unchanged from the original model, never empirically validated) combined with Harman's own already-large bass/pinna shelf meant a genuinely extreme signature (e.g. a basshead IEM with ~+14 dB absolute bass) only showed as ~+5.6 dB *deviation from Harman*, which the gentle decay barely penalized — then that one bad band got diluted by 7 decent ones in the plain weighted average. Simulated k up to 0.30 against the real 8-IEM collection and 56-genre matrix; **k=0.20** was chosen — restores real spread (P10 ~64%, P90 ~81% vs the old 82-91% clump) and correctly demotes bass-heavy/extreme IEMs (one dropped from 56/56 genres above the 70% threshold to 0/56) while well-tuned generalists stay strong. `MATCH_MATRIX_VERSION` bumped to 3 to invalidate the old k=0.08 cached matrix. Note: this did not change the Xenns Top Pro vs Softears Volume S ordering for Rock (Top Pro still ~2.5pts ahead) — that specific ranking question is about absolute-deviation-from-target vs perceived tonal contrast/shape, a deeper modeling question flagged to the user but not addressed here.
